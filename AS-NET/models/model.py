...
class Generator(nn.Module):
    ...
    def __init__(self, args):
        ...
        #
        self.prompt_l_flag = True
        if self.prompt_l_flag:
            self.prompt_learn = PromptLearner()
    ...
    def Eval(self, ST, QRS, QRS_pos):
        ...
        for i in range(QRS.shape[1]):
            ...
            if self.prompt_l_flag:
                QR_EMB_pl_st = self.prompt_learn(memory, QR_EMB)
                QR_EMB = QR_EMB_pl_st
    ...
    def forward(self, ST, QR, QRs=None, QR_pos=None, QRs_pos=None, mode='train'):
        ...
        memory = self.compute_style(ST)  # 90, 8, 512
        ...
        QR_EMB = self.query_embed(QR).permute(1, 0, 2) # 16, 8, 512
        # print(QR_EMB.shape)
        # print(QR_EMB[0])
        # return
        # query_pos = self.pos_encoder(QR_EMB)

        tgt = torch.zeros_like(QR_EMB) # 16, 8, 512
...
class AS_Net(nn.Module):
    ...
    # def save_images_for_HTR_boost(self, path, loader, split='train', HTR_boost_cop = './corpora_english/wordSet.FulTra'):
    # 
    #
    #     # @ym：读取IAM train词库，替代原有label
    #     with open(HTR_boost_cop, 'r') as _f:
    #         HTR_boost_cop_texts = _f.read().split()
    #     HTR_boost_cop_texts = [t.encode('utf-8') for t in HTR_boost_cop_texts]
    #
    #     # @ym：所需要的fid
    #     wid_80wid_select_list = ['000', '005', '006', '007', '008', '012', '013', '017', '018', '019',
    ...
    #     path.mkdir(exist_ok=True, parents=True)
    #
    #     self.fake_base = path / f'HTR_boost_{split}'
    #
    #     self.fake_base.mkdir(exist_ok=True)
    #
    #     print('Saving images...')
    ...
    #             ST = data['simg'].to(self.args.device)
    #             writer_ids = data['wcl'].int().tolist()
    #             # assert writer_ids[0]== writer_ids[-1]
    #             print(str(writer_ids[0]), writer_ids)
    #             # if str(writer_ids[0]) not in wid_80wid_select_list: break
    #             #
    #             # texts = HTR_boost_cop_texts
    #             # self.eval_text_encode, self.eval_len_text, self.eval_encode_pos = self.netconverter.encode(texts)
    #             # self.eval_text_encode = self.eval_text_encode.to(self.args.device).unsqueeze(1)
    #             # self.fakes = self.netG.Eval(ST, self.eval_text_encode, self.eval_encode_pos)
    #             # fake_images = torch.cat(self.fakes, 1).detach().cpu().numpy()
    ...
    #             # real image 是从data中拿出的真实图像，为iam数据集
    #             # """
    #             # for i, (fake, wid) in enumerate(zip(fake_images, writer_ids)):
    #             #
    #             #     lb = ilist[i]
    #             #     big_index = HTR_boost_cop_texts.index(lb)
    #             #     img_id = f'{big_index:07d}_{lb}.png'
    #             #     fake_img_path = self.fake_base / f"{wid:03d}" / img_id
    #             #     fake_img_path.parent.mkdir(exist_ok=True, parents=True)
    #             #     cv2.imwrite(str(fake_img_path), 255 * fake.squeeze())
    #     return
